...
const showInvoices = function () {
  table.innerHTML = "";
  const origin = window.location.origin;
  getJson(`${origin}/api/Invoice`)
    .then((data) => {
          ...
          `
            <tr>
              ...
              <td>
                ...
                <button onclick="passToView(${ele.id})" class="btn view-btn">View Details</button>
              </td>
            </tr>
          `
        );
...
function passToDelete(id) {
  if (confirm(`Are you sure you want to delete this `)) {
    deleteJson(`${origin}/api/Invoice/${id}`, "Cannot Delete Record").catch(
      (err) => console.log(err)
    );
